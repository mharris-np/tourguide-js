...
import {
	arrow,
	autoPlacement,
	computePosition as fui_computePosition,
	offset,
	Placement,
	shift,
	MiddlewareData,
...
async function renderDialogHtml(tgInstance: TourGuideClient) {
	...
	if (tgInstance.options.showButtons && !tgInstance.options.hideNext)
		htmlRes += `<button type="button" class="tg-dialog-btn" id="tg-dialog-next-btn">${tgInstance.options.nextLabel}</button>`
...
function arrowStyles(
	...
	const isInCorner =
		arrowMiddlewareData.centerOffset !== 0 || ((isAtMinWidth || isAtMaxWidth) && (isAtMinHeight || isAtMaxHeight))
